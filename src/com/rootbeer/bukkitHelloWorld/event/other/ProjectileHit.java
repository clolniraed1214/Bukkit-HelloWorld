	...
	public void onProjectileHit(ProjectileHitEvent event) {
		...
		if (p.getType() == EntityType.SNOWBALL) {
			...
			if (arrow.hasMetadata("isBombArrow")) {
				if (arrow.getMetadata("isBombArrow").get(0).asBoolean()) {
					if (arrow.getMetadata("isBombArrow").get(0).asBoolean()) {
						...
						if (!arrow.hasMetadata("deleteOnHit")) {
							if (arrow.getMetadata("hasVariance").get(0).asBoolean())
								destruction = (float) ((float) arrow.getTicksLived() / 7.5);
						}
				...
				ArrowExplode.arrowExplode(plugin, arrow.getLocation(), false, plugin, false, "", false);
			}
		}
	}
}
